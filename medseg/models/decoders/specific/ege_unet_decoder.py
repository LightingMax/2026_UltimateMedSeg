...
class EGEUNetDecoder(nn.Module):
    ...
    def __init__(self, encoder_channels: List[int], bottleneck_channels: int,
        ...
        # 编码器 _ 通道 has 5 items ( after 适配器 ), 映射 to c _ list [ 0: 5 ] / encoder_channels has 5 items (after adapter), map to c_list[0:5]
        ...
        if encoder_channels is not None and len(encoder_channels) == 5:
            ...
            # Also project 瓶颈层 / Also project bottleneck to c_list[5] if needed
        ...
        # GT 深度 supervision side convs ( for GAB 掩码 generation ) / GT deep supervision side convs (for GAB mask generation)
