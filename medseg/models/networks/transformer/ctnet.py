"""CTNet: Contrastive Transformer Network for Polyp Segmentation.
    CTNet: 对比的 Transformer 网络 for 息肉 分割。
...
class _SMIModule(nn.Module):
    """Scale-aware Multi-scale Interaction 模块。
...
class _CIModule(nn.Module):
    """Cross-level Interaction 模块 for 多尺度 融合。
...
class _ProjectionHead(nn.Module):
    """Projection 头部 for 对比的 learning ( upstream 对比的 _ 头部. py )。
    ...
    def forward(self, x):
        # x: ( B, C, H, W ) - > 全局的 avg pool - > project / x: (B, C, H, W) -> global avg pool -> project
...
class CTNet(nn.Module):
    ...
    def __init__(self, in_channels=3, num_classes=2, img_size=352,
        ...
        # MiT-B3 编码器 / MiT-B3 encoder: dims=[64,128,320,512], layers=[3,4,18,3]
        ...
        # Projection 头部 for 对比的 learning / Projection head for contrastive learning
    ...
    def forward(self, x):
        ...
        # 对比的 嵌入 from deepest 特征 / Contrastive embedding from deepest feature
        emb = self.contrasthead(x4)

        # SMI on 深度 特征 / SMI on deep features
        x4 = self.smim(x4)
        x3 = self.smim(x3)
        x2 = self.smim(x2)
