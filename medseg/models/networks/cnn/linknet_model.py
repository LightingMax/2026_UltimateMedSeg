...
class _BasicBlock(nn.Module):
    """Basic residual block for LinkNet 编码器。
...
class _DecoderBlock(nn.Module):
    ...
    def __init__(self, in_ch, out_ch):
        ...
        mid_ch = in_ch // 4
        self.block = nn.Sequential(
            # 1 × 1 reduce 通道 / 1×1 reduce channels
            nn.Conv2d(in_ch, mid_ch, 1, bias=False),
            nn.BatchNorm2d(mid_ch),
            nn.ReLU(inplace=True),
            # 转置 conv for 上采样 / Transpose conv for upsampling
            nn.ConvTranspose2d(mid_ch, mid_ch, 3, stride=2, padding=1,
                               output_padding=1, bias=False),
            nn.BatchNorm2d(mid_ch),
            nn.ReLU(inplace=True),
            # 1 × 1 restore 通道 / 1×1 restore channels
...
class LinkNet(nn.Module):
    ...
    def __init__(self, in_channels=3, num_classes=2, img_size=224):
        ...
        # Initial 卷积 ( 步长 = 2 ) / Initial convolution (stride=2)
        ...
        # 编码器 阶段 ( ResNet18-like ) / Encoder stages (ResNet18-like)
        ...
        # Decoder stages (summation 跳跃连接 / Decoder stages (summation skip connections)
        ...
        # Final full-resolution deconv + 输出 / Final full-resolution deconv + output
    ...
    def forward(self, x):
        ...
        # Initial: / 4 分辨率 / Initial: /4 resolution
        x0 = self.initial(x)
        ...
        # Decoder with summation 跳跃连接 / Decoder with summation skip connections
        d4 = self.dec4(e4) + e3     # /16
        d3 = self.dec3(d4) + e2     # /8
        d2 = self.dec2(d3) + e1     # /4
        d1 = self.dec1(d2)          # /2

        # Final 上采样 to 输入 分辨率 / Final upsampling to input resolution
