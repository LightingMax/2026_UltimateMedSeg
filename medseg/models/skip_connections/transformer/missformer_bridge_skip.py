...
class _EfficientSelfAttention(nn.Module):
    """高效的 自注意力 with 可选 spatial-reduction ( per MISSFormer )。
...
class _MixFFN(nn.Module):
    """DWConv + MLP — MISSFormer's MixFFN_skip adapted to per-pair use.
        DWConv + MLP — MISSFormer's MixFFN _ 跳跃 adapted to per-pair use。
    ...
    def forward(self, x: torch.Tensor, h: int, w: int) -> torch.Tensor:
        ...
        # 残差 + norm ( matching official MixFFN _ 跳跃 ) / Residual + norm (matching official MixFFN_skip)
...
class MISSFormerBridgeSkip(nn.Module):
    """MISSFormer-style joint self-attention bridge skip (per-pair).
        MISSFormer-style joint self-attention bridge 跳跃连接。
    ...
    def _pick_common_dim(self, d_ch: int, s_ch: int) -> int:
        # Use min and round to multiple of num _ heads so 注意力 head-dim is integer / Use min and round to multiple of num_heads so attention head-dim is integer
        c = min(d_ch, s_ch)
    ...
    def forward(self, decoder_feat: torch.Tensor, skip_feat: torch.Tensor) -> torch.Tensor:
        # Align skip spatial to 解码器 / Align skip spatial to decoder
        ...
        # If total 标记 count is too large for joint 自注意力 / If total token count is too large for joint self-attention,
        # fall back to projected concatenation (identity 跳跃连接 / fall back to projected concatenation (identity skip)
        ...
        # 1. Project to common 通道 / 1. Project to common channels
        ...
        # 2. 展平 and concat — both at the same 空间的 分辨率 / 2. Flatten and concat — both at the same spatial resolution
        d_tok = d.flatten(2).transpose(1, 2)  # (B, N, C)
        s_tok = s.flatten(2).transpose(1, 2)  # (B, N, C)
        # For 高效的 SR-attn we need to view the concatenated 标记 as a / For efficient SR-attn we need to view the concatenated tokens as a
        # " 2H x W " grid so the SR conv has well-defined 输入 形状 / "2H x W" grid so the SR conv has well-defined input shape.
        concat = torch.cat([d_tok, s_tok], dim=1)  # (B, 2N, C)

        # 3. Joint 自注意力 + 残差 ( treat as 2H x W 空间的 grid ) / 3. Joint self-attention + residual (treat as 2H x W spatial grid)
        # Pad to make reduction _ 比率 divisible if needed / Pad to make reduction_ratio divisible if needed
        joint_h, joint_w = 2 * H, W
        if self.reduction_ratio > 1:
            pad_h = (-joint_h) % self.reduction_ratio
            pad_w = (-joint_w) % self.reduction_ratio
            if pad_h or pad_w:
                # Pad 标记 in the 空间的 grid / Pad tokens in the spatial grid
        ...
        # Crop back the 填充 if any / Crop back the padding if any
        ...
        # 5. Split back to per-scale 标记; per-scale MixFFN / 5. Split back to per-scale tokens; per-scale MixFFN
        d_split = tx2[:, :N, :]   # decoder tokens
        s_split = tx2[:, N:, :]   # skip tokens
        d_ffn = mod["mixffn_d"](d_split, H, W)
        s_ffn = mod["mixffn_s"](s_split, H, W)

        # 6. Add 残差 ( per scale ) and 重塑 back to BCHW / 6. Add residual (per scale) and reshape back to BCHW
