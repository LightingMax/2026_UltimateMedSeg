...
class CCMSkip(nn.Module):
    """Cross Channel Module: cross-channel interaction between 解码器。
    ...
    def forward(self, decoder_feat, skip_feat):
        # Channel-wise cross 注意力 via 全局的 average 池化 / Channel-wise cross attention via global average pooling
        B, Cd, H, W = decoder_feat.shape
        Cs = skip_feat.shape[1]
        # 全局的 descriptors / Global descriptors
        d_pool = decoder_feat.mean(dim=[2, 3])  # B, Cd
        s_pool = skip_feat.mean(dim=[2, 3])  # B, Cs
        # Cross 注意力 权重 / Cross attention weights
