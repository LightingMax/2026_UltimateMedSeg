...
class DeformableSkip(nn.Module):
    ...
    def _build(self, skip_ch, device):
        ...
        p = self.padding
        if _HAS_DEFORM:
            # Offset conv predicts 2 * k * k offset 通道 per 空间的 loc / Offset conv predicts 2 * k * k offset channels per spatial loc.
            offset_conv = nn.Conv2d(
                skip_ch, 2 * k * k,
                kernel_size=k, padding=p, bias=True,
            )
            # 初始化 offsets to zero so initial behaviour matches a / Initialize offsets to zero so initial behaviour matches a
    ...
    def forward(self, decoder_feat, skip_feat):
        # Spatially align skip to 解码器 / Spatially align skip to decoder if needed.
